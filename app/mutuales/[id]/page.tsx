...
export default function EditarMutualPage() {
  ...
  useEffect(() => {
    ...
    const loadMutual = async () => {
      try {
        ...
        const pacientesResult = await pb.collection("pacientes").getList(1, 1, {
          filter: `mutual_id = "${id}"`,
          requestKey: null,
        });
        setPatientCount(pacientesResult.totalItems);
      } catch (error) {
  ...
  const handleDelete = async () => {
    const message = patientCount > 0
      ? `Esta mutual tiene ${patientCount} pacientes relacionados. ¿Estás seguro de que deseas eliminarla?`
      : "¿Estás seguro de que deseas eliminar esta mutual?";
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">
                {isViewMode ? "Ver Mutual" : "Editar Mutual"}
              </h1>
              <p className="text-zinc-500 dark:text-zinc-400 text-sm">
                {patientCount} pacientes relacionados
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
              ...
              <div>
                ...
                <input type="text" name="codigo" value={formData.codigo} onChange={handleInputChange} disabled={isViewMode} className="w-full px-3 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 dark:text-zinc-200 disabled:opacity-70" />
              </div>

              <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Dirección</label>
                <input type="text" name="direccion" value={formData.direccion} onChange={handleInputChange} disabled={isViewMode} className="w-full px-3 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 dark:text-zinc-200 disabled:opacity-70" />
              </div>

              <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Teléfono</label>
                <input type="text" name="telefono" value={formData.telefono} onChange={handleInputChange} disabled={isViewMode} className="w-full px-3 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 dark:text-zinc-200 disabled:opacity-70" />
              </div>
            </div>
