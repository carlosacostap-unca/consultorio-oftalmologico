...
export function buildRuntimeActivePatientFilter(filter: string, desktopRuntime: boolean) {
  return buildActiveRecordFilter(appendActivePatientFilter(filter), desktopRuntime);
}
